...
deepSleep中に、一定回数ボタンを押下するとWakeUpし、UART[115200]にログを出力する。

カウント数は、RTC Low Memoryに保持しており、電源を落とさない限り、deepSleep中も消えることはない。

また、ULPのアセンブラ側のグローバルシンボルはRTC Low Memoryに保持されており、生成される`ulp_main.h`によりメインプロセッサ側(Cソース側)からも参照が可能である。参照する場合、接頭辞に`ulp_`をつけたものが変数名となる。
